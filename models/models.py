...
class TextWithMetadata(object):
    def __init__(self, searchobject):
        ...
        self.invnr = searchobject["_source"]["metadata"]['inventory_num']
        scan = searchobject["_source"]["columns"][0]['metadata']['scan_id']
        volgnr = scan.split('_')[-1]
        self.volgnr = int(volgnr)
        md = metadata.get('meeting_date')
    ...
    def make_url(self):
        ...
        if self.line_coords != []:
            x = min([c['left'] for c in self.line_coords])
            w = max([c['width'] for c in self.line_coords])
            y = min([c['top'] for c in self.line_coords])
            h = sum([c['height'] for c in self.line_coords])
